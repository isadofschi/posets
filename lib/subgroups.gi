...
function(G,p)
	...
	points:=Set(Filtered(chains, c -> ForAll(c, H-> IsNormal(c[1],H))));
	# Speedup: we encode vertices and then call IsSubset,
	# instead of using IsSubset as the ordering.
	...
	SetOrderingByIndex(RpG,ord_by_index);
...
[IsGroup, IsInt],
function(G,p)
	local S, subs, conj_classes, H, chi, conj_H, n;
	if not (IsPrime(p) and RemInt(Order(G),p)=0) then
		Print("# Error, p must be a prime number dividing the order of G\n");
		return fail;
	fi;
	S :=SylowSubgroup(G,p);
	subs:= Filtered(List(ConjugacyClassesSubgroups(S),Representative), IsElementaryAbelian);
	conj_classes:=[];
	for H in subs do
		if Order(H)>1 then
			if not (H^G in conj_classes) then
				Add(conj_classes,H^G);
